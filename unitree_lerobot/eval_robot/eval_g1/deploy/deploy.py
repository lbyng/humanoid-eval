...
def send_request(image_array: np.ndarray, instruction: str, server_url: str, timeout: float = 5.0) -> List[float]:
    """Send image and instruction to inference server"""
    payload = {
        "image": image_array.tolist(),  # Convert numpy array to list for JSON
    ...
    try:
        response = requests.post(server_url, headers=headers, json=payload, timeout=timeout)
        ...
        # Extract action from response (adjust based on your server's response format)
        if 'action' in result:
            action = result['action']
        else:
            action = result  # Assume the response is the action directly
        
        # Validate action
        if not isinstance(action, list) or len(action) != 16:
            raise ValueError(f"Expected 16-dimensional action, got: {action}")
...
def run_closed_loop_control(
    ...
    # Import configuration for additional parameters
    try:
    ...
    print("[INFO] Starting G1 closed-loop control")
    ...
    try:
        while step < max_steps:
            ...
            try:
                action = send_request(current_image, task_instruction, server_url, timeout=step_timeout)
                ...
                print(f"[ERROR] Failed to get/execute action: {e}")
                continue
            
            # Maintain frequency
            step_time = time.time() - loop_start
            sleep_time = max(0, (1.0 / frequency) - step_time)
            if sleep_time > 0:
...
def main():
    ...
    try:
        ...
        print("[ERROR] deploy_config.py not found. Please create it based on the template.")
        return
    
    # Initialize logging
    init_logging()
    
    print("[INFO] G1 Robot Deployment")
    print("="*60)
