...
def send_request(image_array: np.ndarray, instruction: str, server_url: str, timeout: float = 5.0) -> np.ndarray:
    ...
    try:
        response = requests.post(server_url, headers=headers, data=json_numpy.dumps(payload), timeout=timeout)
    ...
    try:
        while step < max_steps:
            ...
            try:
                ...
                action_chunk = send_request(current_image, task_instruction, server_url, timeout=step_timeout)
